...
public class Runner {
	...
	private static List<Tent> sortTentsByAreas(ArrayList<Tent> tents) {
		return tents.stream().sorted(Comparator.comparing(tent -> tent.getLength() * tent.getWidth()))
				.collect(Collectors.toList());
	}

	// 2
	private static List<Tent> getTentsByNumOfPeople(ArrayList<Tent> tents, int num) {
		return tents.stream().filter(tent -> tent.getNumOfPeople() >= num)
				.sorted((t1, t2) -> t1.numOfPeople - t2.numOfPeople).collect(Collectors.toList());
	}
	...
	// 5
	private static List<Tent> getTentsByHeights(Map<Double, List<Tent>> map, double maxHeight, double minHeight) {
		return map.values().stream().flatMap(x -> x.stream())
				.filter(x -> x.height >= minHeight && x.height <= maxHeight).collect(Collectors.toList());
	}
}
